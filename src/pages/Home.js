...
const Home = () => {
  ...
  useEffect(() => {
    ...
    if (income > 0 && total > income * 0.5) {
      setNotification({ message: '?? Warning: You have spent over 50% of your monthly income!', type: 'warning' });
    }
  ...
  const handleAddFunds = () => {
    const amountValue = parseFloat(addFundsAmount);
    if (isNaN(amountValue) || amountValue <= 0) {
      alert('Please enter a valid amount');
      return;
    }
    ...
    const updatedGoals = savingsGoals.map(goal => {
      if (goal.id === selectedGoalId) {
        return { ...goal, currentAmount: (goal.currentAmount || 0) + amountValue };
      }
      return goal;
    });
    ...
    setNotification({ message: '?? Funds added to savings goal!', type: 'success' });
  };
    ...
    <div style={{ backgroundColor: styles.background, minHeight: '100vh', fontFamily: "'Inter', 'Poppins', sans-serif" }}>
      ...
      <main style={{ maxWidth: '1400px', margin: '0 auto', padding: '2rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
          <div style={{ backgroundColor: styles.cardBg, borderRadius: '16px', padding: '1.25rem', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
          ...
          <div style={{ backgroundColor: styles.cardBg, borderRadius: '16px', padding: '1.25rem', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
            ...
            <IncomeExpenseBarChart income={income} expenses={totalExpenses} />
          </div>
        </div>

        {/* Savings Goal Progress with Add Funds Button */}
        {mainSavingsGoal && (
          <div style={{ backgroundColor: styles.cardBg, borderRadius: '16px', padding: '1.25rem', marginBottom: '2rem', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
              <h3 style={{ color: styles.text, margin: 0, fontSize: '1rem' }}>?? Savings Goal: {mainSavingsGoal.name}</h3>
              <span style={{ color: '#3B82F6', fontSize: '0.75rem', fontWeight: '600' }}>{savingsPercentage.toFixed(1)}% Complete</span>
            </div>
            <div style={{ height: '8px', backgroundColor: styles.border, borderRadius: '8px', overflow: 'hidden', marginBottom: '0.5rem' }}>
              <div style={{ width: Math.min(savingsPercentage, 100) + '%', height: '100%', backgroundColor: savingsPercentage >= 100 ? '#4CAF50' : '#3B82F6', transition: 'width 0.3s ease' }}></div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
            ...
            <div style={{ marginTop: '0.5rem', padding: '0.5rem', backgroundColor: styles.background, borderRadius: '8px', marginBottom: '0.5rem' }}>
              <span style={{ color: '#3B82F6', fontSize: '0.7rem' }}>
                {savingsPercentage >= 100 ? '?? Congratulations! Goal achieved! ??' : ?? You're % there! Only  more to go!}
              </span>
            </div>
            <button
              onClick={() => {
                setSelectedGoalId(mainSavingsGoal.id);
        ...
        {/* Filters and Search */}
        <div style={{ backgroundColor: styles.cardBg, borderRadius: '16px', padding: '1rem', marginBottom: '1.5rem', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              <input
                ...
                style={{ padding: '0.5rem 0.75rem', borderRadius: '8px', border: '1px solid ' + styles.border, backgroundColor: styles.background, color: styles.text, fontSize: '0.85rem', width: '180px', transition: 'all 0.2s' }}
              />
            ...
            <div style={{ position: 'relative' }}>
              <button onClick={() => setShowExportMenu(!showExportMenu)} style={{ padding: '0.5rem 1rem', backgroundColor: '#3B82F6', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '0.85rem', transition: 'all 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#2563EB'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#3B82F6'}>?? Export CSV</button>
              {showExportMenu && (
        ...
        {/* Expense List - Grouped with Icons */}
        <div style={{ backgroundColor: styles.cardBg, borderRadius: '16px', padding: '1.25rem', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
          ...
          {Object.keys(groupedExpenses).length === 0 ? (
            <div style={{ textAlign: 'center', padding: '3rem', color: styles.textSecondary }}>
              ...
              <p style={{ fontSize: '1rem' }}>No expenses yet</p>
              <p style={{ fontSize: '0.85rem', marginTop: '0.5rem' }}>Click the + button to add your first expense</p>
            </div>
          ) : (
            Object.entries(groupedExpenses).map(([category, data]) => (
              <div key={category} style={{ marginBottom: '1rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem', backgroundColor: styles.background, borderRadius: '12px', marginBottom: '0.5rem', transition: 'all 0.2s' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <span style={{ fontSize: '1.2rem' }}>{categoryIcons[category] || '??'}</span>
                    <div>
                      <div style={{ fontWeight: '600', color: styles.text }}>{category}</div>
                      <div style={{ fontSize: '0.7rem', color: styles.textSecondary }}>{data.count} transaction{data.count !== 1 ? 's' : ''}</div>
                    </div>
                  </div>
                  <div style={{ fontWeight: 'bold', color: '#DC2626', fontSize: '1rem' }}>{formatAmount(data.total)}</div>
                </div>
                {data.items.map(expense => (
                  <div key={expense._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 0.5rem 0.5rem 2rem', borderBottom: '1px solid ' + styles.border, transition: 'all 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = styles.background} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
                    <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      ...
                      <button onClick={() => handleDelete(expense._id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: styles.textSecondary, fontSize: '0.85rem', transition: 'color 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.color = '#DC2626'} onMouseLeave={(e) => e.currentTarget.style.color = styles.textSecondary}>Delete</button>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {/* Floating Action Button - Phase 2 */}
      <button
      ...
      {modal === 'expense' && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
          <div style={{ backgroundColor: styles.cardBg, borderRadius: '20px', padding: '1.5rem', width: '90%', maxWidth: '450px', animation: 'fadeIn 0.2s ease' }}>
            <h2 style={{ color: styles.text, marginBottom: '1rem', fontSize: '1.25rem' }}>? Add New Expense</h2>
            <form onSubmit={handleAddExpense} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <input type="text" placeholder="Title" value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} required style={{ padding: '0.75rem', borderRadius: '10px', border: '1px solid ' + styles.border, backgroundColor: styles.background, color: styles.text, transition: 'all 0.2s' }} />
              ...
              <select value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})} style={{ padding: '0.75rem', borderRadius: '10px', border: '1px solid ' + styles.border, backgroundColor: styles.background, color: styles.text }}>
                {categories.filter(c => c !== 'All').map(cat => <option key={cat} value={cat}>{categoryIcons[cat]} {cat}</option>)}
              </select>
              ...
              <div style={{ display: 'flex', gap: '0.75rem' }}>
                <button type="submit" style={{ flex: 1, padding: '0.75rem', backgroundColor: '#1E3A8A', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', transition: 'all 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#2563EB'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#1E3A8A'}>Save</button>
      ...
      {showAddFundsModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
          <div style={{ backgroundColor: styles.cardBg, borderRadius: '20px', padding: '1.5rem', width: '90%', maxWidth: '350px' }}>
            <h2 style={{ color: styles.text, marginBottom: '1rem' }}>?? Add Funds to Savings</h2>
            <input type="number" step="0.01" placeholder="Amount to add (PHP)" value={addFundsAmount} onChange={(e) => setAddFundsAmount(e.target.value)} style={{ width: '100%', padding: '0.75rem', borderRadius: '10px', border: '1px solid ' + styles.border, backgroundColor: styles.background, color: styles.text, marginBottom: '1rem' }} />
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <button onClick={handleAddFunds} style={{ flex: 1, padding: '0.75rem', backgroundColor: '#3B82F6', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer' }}>Add Funds</button>
              <button onClick={() => { setShowAddFundsModal(false); setAddFundsAmount(''); setSelectedGoalId(null); }} style={{ flex: 1, padding: '0.75rem', backgroundColor: '#6B7280', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer' }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modal === 'goal' && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
          <div style={{ backgroundColor: styles.cardBg, borderRadius: '20px', padding: '1.5rem', width: '90%', maxWidth: '350px' }}>
            ...
            <input type="text" placeholder="Goal Name (e.g., New Phone)" value={goalForm.name} onChange={(e) => setGoalForm({...goalForm, name: e.target.value})} style={{ width: '100%', padding: '0.75rem', borderRadius: '10px', border: '1px solid ' + styles.border, backgroundColor: styles.background, color: styles.text, marginBottom: '0.75rem' }} />
            ...
            <input type="text" placeholder="Description (optional)" value={goalForm.description} onChange={(e) => setGoalForm({...goalForm, description: e.target.value})} style={{ width: '100%', padding: '0.75rem', borderRadius: '10px', border: '1px solid ' + styles.border, backgroundColor: styles.background, color: styles.text, marginBottom: '1rem' }} />
            <div style={{ display: 'flex', gap: '0.75rem' }}>
